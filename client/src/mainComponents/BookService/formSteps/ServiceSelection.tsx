...
export const ServiceSelection = ({ form }: ServiceSelectionProps) => {
  ...
  const renderService = (service: (typeof serviceTypes)[0]) => {
    const isUsed = isServiceUsed(service.id);
    const isSelected = watchedValues.serviceType === service.id;
    ...
    return (
      <div
        ...
        onClick={() => !isUsed && setValue("serviceType", service.id)}
      >
        <div className='flex items-start gap-2'>
          <input
            ...
            checked={isSelected}
            disabled={isUsed}
            onChange={() => setValue("serviceType", service.id)}
            className='mt-1 accent-red-600 cursor-pointer disabled:cursor-not-allowed'
          />
          <div className='flex-1'>
            ...
            {isUsed && (
              <p className='text-xs text-red-500 mt-1'>
                This service has already been used and cannot be booked again.
              </p>
            )}
          </div>
        </div>
      ...
      <div className='space-y-2'>
        ...
        <div className='grid grid-cols-1 gap-3'>
          <div className='space-y-3'>
            ...
            {serviceTypes.filter((s) => s.type === "free").map(renderService)}
          </div>
          <div className='space-y-3'>
            <h4 className='text-sm font-medium text-blue-600'>Paid Services</h4>
            {serviceTypes.filter((s) => s.type === "paid").map(renderService)}
          </div>
        </div>
        {errors.serviceType && (
