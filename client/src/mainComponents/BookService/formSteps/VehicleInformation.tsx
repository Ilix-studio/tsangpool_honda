      ...
      <div className='space-y-2'>
        ...
        >
          <option value='' disabled>
            {isLoading ? "Loading your vehicle..." : "Select your motorcycle"}
          </option>
          {vehicle && (
            <option value={vehicle.vehicleId}>{vehicle.modelName}</option>
          )}
        </select>
        {errors.bikeModel && (
      ...
      {!isLoading && !vehicle && (
        <div className='p-4 bg-amber-50 rounded-lg'>
          <p className='text-sm text-amber-600'>
            No vehicle found in your account. Please add a vehicle to your
            profile first.
          </p>
        </div>
      )}
